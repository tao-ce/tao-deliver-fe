...
<script>
    ...
    function draw(groupEl) {
        ...
        if (coords && coords.length) {
            ...
            const outerRadiusOffset = remToPx(0.25);
            const innerRadiusOffset = remToPx(0.625);

            // apply minSize if needed
            // include offets into comparison to avoid negative values when rendered
            const minRadiusPx = minSizePx / 2;
            radius = Math.max(minRadiusPx, radius, outerRadiusOffset, innerRadiusOffset);
            //outer white circle
            groupEl
                .circle((radius - outerRadiusOffset) * 2)
                .attr({ cx, cy })
                .addClass('shape-outer-border');
            //inner black circle
            groupEl
                .circle((radius - innerRadiusOffset) * 2)
                .attr({ cx, cy })
