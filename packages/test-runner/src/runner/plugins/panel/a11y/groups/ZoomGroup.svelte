...
<script>
    ...
    import PageZoomSetting from '../controls/PageZoomSetting.svelte';
    import { getIsNonDefaultState } from '../util.js';
    import { Icon } from '@oat-sa-private/ui-elements';
    import { __ } from '@oat-sa-private/ui-core';

    const groupKey = settingsGroupKeys.zoom;
    ...
    let nonDefault = getIsNonDefaultState(settingsKeys.pageZoom, initialSettingsState, pluginConfig);
...
<Details
    ...
    outlined={nonDefault}
    ariaLabelCollapse={__('collapse zoom settings')}
    ariaLabelExpand={__('expand zoom settings')}>
    <h3 class="ui-heading flex" slot="summary">
        <span>
            <Icon name="search-16" ariaHidden />
            {__('Zoom')}
            {#if nonDefault}
                <CheckMark />
            {/if}
        </span>
        {#if pluginConfig[settingsKeys.pageZoom]?.enabled}
            <PageZoomSetting
                config={pluginConfig[settingsKeys.pageZoom]}
                initialState={initialSettingsState[settingsKeys.pageZoom]?.toolState}
